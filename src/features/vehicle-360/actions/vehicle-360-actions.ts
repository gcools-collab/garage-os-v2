...
export async function uploadVehicle360Frames(vehicleId: string, formData: FormData) {
  const session = await editableSession()
  ...
  for (const file of files.slice(0, Math.max(0, 48 - sequence.frames.length))) {
    const extension = MIME_EXTENSIONS[file.type]
    if (!extension || file.size > 15 * 1024 * 1024) continue
    ...
    const { error } = await supabase.from("vehicle_360_frames").insert({ id: frameId, garage_id: session.garageId, vehicle_id: vehicleId, sequence_id: sequence.id, storage_path: path, position, status: "READY", file_size: file.size, mime_type: file.type })
    if (error) await supabase.storage.from("vehicle-360").remove([path])
    else position += 1
  }
