...
export function Vehicle360ViewerClient({ viewer }: { readonly viewer: Vehicle360ViewerViewModel }) {
  ...
  return (
    <section
      ...
      tabIndex={0}
      onKeyDown={(event) => { if (event.key === "ArrowLeft") move(-1); if (event.key === "ArrowRight") move(1); if (event.key === "Escape") setFullscreen(false) }}
      onPointerDown={(event) => { startX.current = event.clientX; event.currentTarget.setPointerCapture(event.pointerId) }}
      onPointerUp={(event) => { if (startX.current !== null && Math.abs(event.clientX - startX.current) >= 32) move(event.clientX < startX.current ? 1 : -1); startX.current = null }}
    ...
    >
      ...
      <div className={fullscreen ? "relative aspect-[4/3] w-full max-w-6xl" : "relative aspect-[4/3] w-full"}>
        <AssetImage image={frame.image} sizes={fullscreen ? "100vw" : "(max-width: 1024px) 100vw, 900px"} />
      </div>
      <div className="absolute inset-x-3 bottom-3 flex items-center justify-between gap-2 rounded-xl bg-background/90 p-2 backdrop-blur">
        <button type="button" aria-label={viewer.previousLabel} onClick={() => move(-1)} className="rounded-md p-2 focus-visible:outline-2"><ChevronLeft /></button>
        <span aria-live="polite" className="text-sm tabular-nums">{index + 1} / {viewer.frames.length}</span>
        <div className="flex gap-1">
          <button type="button" aria-label={viewer.resetLabel} onClick={() => setIndex(viewer.startIndex)} className="rounded-md p-2 focus-visible:outline-2"><RotateCcw /></button>
          <button type="button" aria-label={fullscreen ? "Fermer le plein écran" : viewer.fullscreenLabel} onClick={() => setFullscreen((value) => !value)} className="rounded-md p-2 focus-visible:outline-2">{fullscreen ? <X /> : <Maximize2 />}</button>
          <button type="button" aria-label={viewer.nextLabel} onClick={() => move(1)} className="rounded-md p-2 focus-visible:outline-2"><ChevronRight /></button>
        </div>
      </div>
    </section>
  )
