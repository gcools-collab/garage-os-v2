    ...
    <PublicSiteLayout garage={contact.garage}>
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-16 lg:grid-cols-2 md:px-8">
        ...
        <form className="space-y-5 rounded-2xl border border-[var(--live-border)] p-6" aria-label="Formulaire de contact">
          ...
          <button type="button" disabled className="min-h-11 rounded-lg bg-[var(--live-primary)] px-5 text-[var(--live-primary-foreground)] disabled:opacity-60">{contact.form.submitLabel}</button>
          <p className="text-xs text-[var(--live-muted-foreground)]">Le formulaire sera prochainement connecté au suivi commercial Garage OS.</p>
        </form>
      </div>
    </PublicSiteLayout>
  )
