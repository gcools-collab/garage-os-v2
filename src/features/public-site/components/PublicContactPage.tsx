import type { PublicRequestFormViewModel, PublicRequestSource } from "@/features/public-leads"
import { PublicRequestForm } from "@/features/public-leads"
import type { PublicContactViewModel } from "../types"
import { PublicSiteLayout } from "./PublicSiteLayout"

export function PublicContactPage({ contact, request, unavailableRequest = false }: {
  readonly contact: PublicContactViewModel
  readonly request?: { readonly form: PublicRequestFormViewModel; readonly vehicleSlug: string | null; readonly source: PublicRequestSource } | null
  ...
  return <PublicSiteLayout garage={contact.garage}>
    ...
    {request ? <div className="mx-auto max-w-4xl px-5 pb-16 md:px-8"><PublicRequestForm form={request.form} garageSlug={contact.garage.slug} vehicleSlug={request.vehicleSlug} source={request.source} publicPageUrl={`${contact.garage.homeHref}/contact`} /></div> : unavailableRequest ? <p role="status" className="mx-auto max-w-4xl px-5 pb-16 text-[var(--live-muted-foreground)] md:px-8">Cette demande n’est pas disponible pour ce garage. Vous pouvez choisir un autre moyen de contact.</p> : null}
  </PublicSiteLayout>
}
