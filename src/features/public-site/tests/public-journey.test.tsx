...
test("navigation, services, location et dépôt-vente utilisent leurs ViewModels", () => {
  ...
  assert.match(servicesHtml, /39,90 € TTC/)
  assert.match(servicesHtml, /49,90 € TTC/)
  assert.match(servicesHtml, /Jusqu’à 1,9 L/)
  assert.match(servicesHtml, /2 L et plus/)
  assert.match(servicesHtml, /\+19,90 € jusqu’à 1,9 L/)
  assert.match(servicesHtml, /\+29,90 € à partir de 2 L/)
  assert.match(servicesHtml, /Option : 30 €/)
  assert.doesNotMatch(servicesHtml, /Passage de la valise/)
  assert.doesNotMatch(servicesHtml, /sm:col-span-2/)
  assert.match(servicesHtml, /panne mécanique/)
  assert.match(servicesHtml, /FAP défectueux/)
  assert.doesNotMatch(servicesHtml, /90€ TTC chez la concurrence|plus de 75%/)
...
test("la location expose durée, documents, conditions, téléphone et itinéraire", () => {
  ...
  assert.match(html, /src="\/partners\/cargo-location\.png"/)
  assert.match(html, /Location de véhicules/)
  assert.match(html, /Réserver en ligne/)
  assert.match(html, /Demander un devis/)
  assert.doesNotMatch(html, /Visuel CarGo/)
  assert.doesNotMatch(html, /Réservez en ligne chez Cargo/)
  assert.doesNotMatch(html, /_next\/image/)
  assert.doesNotMatch(html, /\+500|11000|91%|35 ANS/)
