  ...
  return (
    <>
      <VehicleSection id="vehicle-services" title="Services proposés">
        <div className="grid gap-4 sm:grid-cols-2">{detail.services.map((service) => <article key={service.id} className="rounded-xl border border-[var(--live-border)] p-4"><h3 className="font-semibold">{service.title}</h3><p className="mt-2 text-sm text-[var(--live-muted-foreground)]">{service.description}</p></article>)}</div>
      </VehicleSection>
