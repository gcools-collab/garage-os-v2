...
import { InteriorTourViewerClient, type InteriorTourViewerViewModel } from "@/features/interior-tour"

export function PremiumVehicleDetailPage({ detail, vehicle360 = null, interiorTour = null }: { readonly detail: VehicleDetailPageViewModel; readonly vehicle360?: Vehicle360ViewerViewModel | null; readonly interiorTour?: InteriorTourViewerViewModel | null }) {
  return (
    <PublicSiteLayout garage={detail.garage}>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 pb-28 lg:grid-cols-[minmax(0,1fr)_20rem] lg:pb-20 md:px-8">
        <div className="space-y-8">
          ...
          {vehicle360 || interiorTour ? <section className="rounded-2xl border border-[var(--live-border)] bg-[var(--live-surface)] p-5 md:p-7"><div className="mb-5"><p className="text-sm font-medium text-[var(--live-primary)]">Expérience immersive</p><h2 className="mt-1 text-2xl font-semibold">Visite virtuelle</h2><p className="mt-2 text-sm text-[var(--live-muted-foreground)]">Explorez le véhicule sous tous les angles. Utilisez la galerie classique à tout moment.</p></div><div className="space-y-6">{vehicle360 ? <Vehicle360ViewerClient viewer={vehicle360} /> : null}{interiorTour ? <InteriorTourViewerClient viewer={interiorTour} /> : null}</div></section> : null}
