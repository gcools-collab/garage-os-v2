...
test("la reprise revalide le token et dérive le rendez-vous côté serveur", () => {
  ...
  assert.match(source, /startAppointmentPayment\(String\(portal\.appointment\.id\), garageSlug\)/)
...
test("la reprise conserve les protections d'idempotence, de montant et de mode TEST", () => {
  const paymentAction = readFileSync("src/features/payments/services/payment-creation-service.ts", "utf8")
...
test("aucun identifiant de rendez-vous arbitraire n'entre dans la reprise publique", () => {
  const action = readFileSync("src/features/registration/actions/public-registration-actions.ts", "utf8")
  const component = readFileSync("src/features/registration/components/public-registration-payment-resume.tsx", "utf8")
  assert.match(action, /getPublicRegistrationPortal\(garageSlug, token\)/)
  assert.match(action, /portal\.appointment\.id/)
  assert.doesNotMatch(action, /formData\.get\("appointmentId"\)/)
  assert.doesNotMatch(component, /name=["']appointmentId["']/)
})
