    ...
    <form ref={formRef} action={action} className="mt-6 space-y-7">
      ...
      {offers.length ? (
        <fieldset className="grid gap-5 border-0 p-0 sm:grid-cols-2">
          ...
          <PublicOfferSelector offers={offers} onOfferChange={setSelectedOfferSlug} />
        </fieldset>
      ) : null}
      ...
      {visibleSlots.length ? <PublicSlotSelector key={selectedOfferSlug} slots={visibleSlots} /> : baseAvailability.length || availability.length ? <p className="rounded-lg border p-4 text-sm text-[var(--live-muted-foreground)]">Aucun créneau compatible avec la durée de cette prestation.</p> : null}
