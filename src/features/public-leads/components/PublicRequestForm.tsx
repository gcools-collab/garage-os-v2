...
export function PublicRequestForm({ form, garageSlug, vehicleSlug, vehicleContext, source, publicPageUrl, availability = [] }: {
  ...
  if (state.status === "success") return <section aria-live="polite" className="rounded-2xl border border-[var(--live-border)] p-6"><h2 className="text-2xl font-semibold">Demande transmise</h2><p className="mt-3">{state.message}</p>{state.reference ? <p className="mt-3 font-medium">Référence {state.reference}</p> : null}{state.paymentUrl?<a href={state.paymentUrl} className="mt-5 inline-flex min-h-12 items-center rounded-lg bg-[var(--live-primary)] px-6 font-semibold text-[var(--live-primary-foreground)]">Accéder au paiement sécurisé</a>:state.appointmentStatus==="AWAITING_PAYMENT"?<p className="mt-4 text-sm text-[var(--live-muted-foreground)]">Le paiement en ligne est temporairement indisponible. Le garage vous recontactera.</p>:null}</section>
