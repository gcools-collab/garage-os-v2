...
const definitions: Readonly<Record<PublicRequestType, Omit<PublicRequestFormViewModel, "type" | "contextHeading">>> = {
  ...
  TEST_DRIVE: { title: "Demander un essai", description: "Cette demande doit être confirmée par l’équipe. Le créneau indiqué n’est jamais définitivement réservé tant que le garage ne vous a pas recontacté.", submitLabel: "Demander un essai", steps: steps(["request", "Votre disponibilité"], ["contact", "Vos coordonnées"]), fields: [field("request", { name: "preferredDate", label: "Date souhaitée", type: "date", required: false }), field("request", { name: "preferredTime", label: "Moment préféré", type: "select", required: false, options: [{ value: "MORNING", label: "Matin" }, { value: "AFTERNOON", label: "Après-midi" }] }), message("request"), ...identity()] },
  ...
  REGISTRATION: { title: "Préparer une demande d’immatriculation", description: "Vous pourrez transmettre les documents nécessaires lors de l’étape suivante du parcours.", submitLabel: "Préparer ma demande d’immatriculation", steps: steps(["procedure", "Votre démarche"], ["vehicle", "Votre véhicule"], ["contact", "Vos coordonnées"]), fields: [field("procedure", { name: "procedure", label: "Type de démarche", type: "select", required: true, options: [{ value: "CHANGE_OWNER", label: "Changement de titulaire" }, { value: "DUPLICATE", label: "Duplicata après perte" }, { value: "ADDRESS_CHANGE", label: "Changement d’adresse" }, { value: "IMPORT", label: "Première immatriculation française" }, { value: "TEMPORARY_REGISTRATION", label: "WW provisoire" }, { value: "OTHER", label: "Autre" }] }), field("vehicle", { name: "registration", label: "Immatriculation", type: "text", required: false }), message("vehicle", "Commentaire"), ...identity()] },
