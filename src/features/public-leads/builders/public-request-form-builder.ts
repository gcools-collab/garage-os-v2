...
const definitions: Readonly<Record<PublicRequestType, Omit<PublicRequestFormViewModel, "type" | "contextHeading">>> = {
  ...
  ENGINE_CLEANING: { title: "Demander un décalaminage", description: "Choisissez votre prestation et un créneau disponible.", submitLabel: "Demander mon rendez-vous de décalaminage", steps: steps(["request", "Votre besoin"], ["contact", "Vos coordonnées"]), fields: [field("request", { name: "vehicle", label: "Marque et modèle", type: "text", required: true }), field("request", { name: "reason", label: "Symptôme ou raison", type: "textarea", required: false }), ...identity()] },
