...
async function load(params: Props["params"]) {
  ...
  return { garageSlug, record, page: record ? buildPublicProgram(record.garage, "CONSIGNMENT") : null }
}
...
export default async function ConsignmentRoute({ params }: Props) {
  ...
  if (!page) {
    logPublicRouteDiagnostic({ route: "/g/[garageSlug]/depot-vente", slug: garageSlug, garageId: record?.garage.garageId ?? null, liveSlug: record?.garage.garageSlug ?? null, activeGarageId: null, serviceCount: record?.garage.serviceConfigurations?.length ?? 0, repositoryResult: record ? "FOUND" : "NOT_FOUND", reason: record ? "consignment_service_disabled" : "public_garage_missing" })
    notFound()
  }
